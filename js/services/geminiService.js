...
import { IMAGES, uniqueId, COSTS } from "../constants.js";

// Helper to determine difficulty scaler
const getScaler = (level) => {
  return {
...
// Helper to get random local image
const getRandomImage = () => {
  const options = Object.values(IMAGES);
  return options[Math.floor(Math.random() * options.length)];
};

export const generateEnemyWave = async (level, existingApiKey) => {
  // In static build, we look at window.process.env
  const apiKey = (window.process && window.process.env && window.process.env.API_KEY) || existingApiKey;
  
  // Fallback for procedural generation if no key or error
  if (!apiKey) {
  ...
  try {
    ...
    return rawEnemies.map((e, index) => ({
      id: `enemy-l${level}-${index}-${uniqueId()}`,
      name: e.name,
      description: e.description,
      image: getRandomImage(), // Use local images
      stats: {
        hp: e.hp,
        maxHp: e.hp,
        atk: e.atk,
...
const generateProceduralWave = (level) => {
...
// Procedural shop generation for speed
export const generateShopInventory = (level) => {
    const count = 3;
    const scaler = getScaler(level);
    const shopItems = [];
    const roles = [
    ...
    for(let i=0; i<count; i++) {
        ...
        // Randomize stats around level scaler
...
export const generateRewardCard = async (level, existingApiKey) => {
  const apiKey = (window.process && window.process.env && window.process.env.API_KEY) || existingApiKey;
  const scaler = getScaler(level);
  
  // Boost stats for rewards so they are valuable
  scaler.atk += 2;
  scaler.hp += 10;
